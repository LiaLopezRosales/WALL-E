...
{   //Modificar para poder acceder a las localizaciones de los errores semánticos
    ...
    public EvaluationContext _context { get; set; }
    public FigureRepository _figures { get; set; }
    public RenderScene _scene { get; set; }
    public List<Error> Semantic_Errors { get; set; }
    public string file{get;set;}
    public string line{get;set;}
    public CancellationToken CancellationToken { get; set; }

    public Evaluator(EvaluationContext context, FigureRepository figures, RenderScene scene, string f)
    {
        _context = context;
        _figures = figures;
        _scene = scene;
    ...
    public EvaluationContext ResultingContext()
    {
        return _context;
    }
    ...
    public object GeneralEvaluation(Node node)
    {
        ...
        if (node.Type == Node.NodeType.Circle)
        {
            ...
            {
                if (_context.GlobalConstant.ContainsKey(node.NodeExpression!.ToString()!))
                {
                ...
                else _context.GlobalConstant.Add(node.NodeExpression!.ToString()!, c);
            }
            else
            {
                if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && !CurrentScope.InFunction)
                {
                    Semantic_Errors.Add(new Error(Error.TypeError.Semantic_Error, Error.ErrorCode.Invalid, "operation,constants can't be modified", new Location(file, line, "column")));
                }
                else if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && CurrentScope.InFunction)
                {
        ...
        else if (node.Type == Node.NodeType.Point)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                if (_context.GlobalConstant.ContainsKey(node.NodeExpression!.ToString()!))
                {
                    ...
                    _context.GlobalConstant.Add(node.NodeExpression!.ToString()!, p);
            }
            else
            {
                if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && !CurrentScope.InFunction)
        ...
        else if (node.Type == Node.NodeType.Line)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                if (_context.GlobalConstant.ContainsKey(node.NodeExpression!.ToString()!))
                {
                    ...
                    _context.GlobalConstant.Add(node.NodeExpression!.ToString()!, l);
            }
            else
            {
                if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && !CurrentScope.InFunction)
        ...
        else if (node.Type == Node.NodeType.Segment)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                if (_context.GlobalConstant.ContainsKey(node.NodeExpression!.ToString()!))
                {
                    ...
                    _context.GlobalConstant.Add(node.NodeExpression!.ToString()!, s);
            }
            else
            {
                if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && !CurrentScope.InFunction)
        ...
        else if (node.Type == Node.NodeType.Ray)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                if (_context.GlobalConstant.ContainsKey(node.NodeExpression!.ToString()!))
                {
                    ...
                    _context.GlobalConstant.Add(node.NodeExpression!.ToString()!, r);
            }
            else
            {
                if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && !CurrentScope.InFunction)
        ...
        else if (node.Type == Node.NodeType.Point_Seq)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                if (_context.GlobalConstant.ContainsKey(node.NodeExpression!.ToString()!))
                {
                    ...
                    _context.GlobalConstant.Add(node.NodeExpression!.ToString()!, pts);
            }
            else
            {
                if (CurrentScope.Variables.ContainsKey(node.NodeExpression!.ToString()!) && !CurrentScope.InFunction)
        ...
        else if (node.Type == Node.NodeType.GlobalVar)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                if (_context.GlobalConstant.ContainsKey(name))
                {
                    ...
                    _context.GlobalConstant.Add(name, value);
            }
            else
            {
                ...
                else if (CurrentScope.Variables.ContainsKey(name) && CurrentScope.InFunction)
                {
        ...
        {
            if (_scene.UtilizedColors.Peek() != node.NodeExpression!.ToString()!)
            {
                _scene.UtilizedColors.Push(node.NodeExpression!.ToString()!);
            }
        ...
        else if (node.Type == Node.NodeType.Restore)
        {
            ...
            return $"Used color has been restore to {_scene.UtilizedColors.Peek()}";
        }
        else if (node.Type == Node.NodeType.Draw)
        {
            ...
            DrawObject d = new DrawObject(value, tag, _scene.UtilizedColors.Peek());
        ...
        else if (node.Type == Node.NodeType.Fuction)
        {
            ...
            if (CurrentScope.Parent == null)
            {
                ...
                else _context.Available_Functions.Add(func);
                        ...
                        if (valueofarg == default(object))
                        {
                            ...
                            if (CurrentScope.Parent == null)
                            {
                                if (_context.GlobalConstant.ContainsKey(name))
                                {
                                    ...
                                    _context.GlobalConstant.Add(name, valueofarg);
                            }
                            else
                            {
                                if (CurrentScope.Variables.ContainsKey(name) && !CurrentScope.InFunction)
                                {
                                    Semantic_Errors.Add(new Error(Error.TypeError.Semantic_Error, Error.ErrorCode.Invalid, "operation,constants can't be modified", new Location(file, line, "column")));
                                }
                                else if (CurrentScope.Variables.ContainsKey(name) && CurrentScope.InFunction)
                    ...
                    else if (subnode.Type == Node.NodeType.VarName)
                    {
                        ...
                        if (CurrentScope.Parent == null)
                        {
                            if (_context.GlobalConstant.ContainsKey(name))
                            {
                                ...
                                _context.GlobalConstant.Add(name, "undefined");
                        }
                        else
                        {
                            if (CurrentScope.Variables.ContainsKey(name) && !CurrentScope.InFunction)
        ...
        else if (node.Type == Node.NodeType.Var)
        {
            ...
            else return _context.GlobalConstant[node.NodeExpression!.ToString()!];
        ...
        else if (node.Type == Node.NodeType.Sin)
        {
            ...
            else return _context.Trig_functions["sin"](Convert.ToDouble(arg, CultureInfo.InvariantCulture));
        }
        else if (node.Type == Node.NodeType.Cos)
        {
            ...
            else return _context.Trig_functions["cos"](Convert.ToDouble(arg, CultureInfo.InvariantCulture));
        }
        else if (node.Type == Node.NodeType.Sqrt)
        {
            ...
            else return _context.Trig_functions["sqrt"](Convert.ToDouble(arg, CultureInfo.InvariantCulture));
        }
        else if (node.Type == Node.NodeType.Log)
        {
            ...
            else return _context.Log["log"](Convert.ToDouble(base_of, CultureInfo.InvariantCulture), Convert.ToDouble(arg, CultureInfo.InvariantCulture));
        }
        else if (node.Type == Node.NodeType.PI)
        {
            return _context.Math_value["PI"]();
        }
        else if (node.Type == Node.NodeType.E)
        {
            return _context.Math_value["E"]();
        }
        ...
        else if (node.Type == Node.NodeType.Points)
        {
            ...
            if (arg is Circle)
            {
                IEnumerable<Point> point = _context.Points["points"]((Circle)arg);
        ...
        else if (node.Type == Node.NodeType.Circle_Fuc)
        {
            ...
            else
            {
                if (radio is Measure)
                {
                    ...
                    _figures.TryAddExistingCircle(temp);
                    return temp;
                }
                else
                {
        ...
        else if (node.Type == Node.NodeType.Line_Fuc)
        {
            ...
            else
            {
                ...
                _figures.TryAddExistingLine(temp);
                return temp;
            }
        }
        else if (node.Type == Node.NodeType.Segment_Fuc)
        {
            ...
            else
            {
                ...
                _figures.TryAddExistingSegment(temp);
                return temp;
            }
        }
        else if (node.Type == Node.NodeType.Ray_Fuc)
        {
            ...
            else
            {
                ...
                _figures.TryAddExistingRay(temp);
                return temp;
            }
        }
        ...
        else if (node.Type == Node.NodeType.Declared_Fuc)
        {
            ...
            if (exist)
            {
                ...
                for (int i = 0; i < _context.Available_Functions.Count; i++)
                {
                    ...
                    if (_context.Available_Functions[i].Name == dfunc_name)
                    {
                        //Se comprueba si tienen la misma cantidad de argumentos
                        if (_context.Available_Functions[i].Functions_Arguments.Count == func_parameters.Branches.Count)
                        {
                            int param_number = 0;
                            foreach (var p_name in _context.Available_Functions[i].Functions_Arguments.Keys)
                            {
                                //Se asignan los argumentos dados a su correspondiente en los declarados por la función
                                _context.Available_Functions[i].Functions_Arguments[p_name] = func_parameters.Branches[param_number];
                            ...
                            {
                                Semantic_Errors.Add(new Error(Error.TypeError.Semantic_Error, Error.ErrorCode.Invalid, $"call,full stack for function {_context.Available_Functions[i].Name} ", new Location(file, line, "column")));
                                return "";
                            }
                            //Se evalua la función solicitada
                            object value = GeneralEvaluation(_context.Available_Functions[index].Code);
                        ...
                        else
                        {
                            ...
                            Semantic_Errors.Add(new Error(Error.TypeError.Semantic_Error, Error.ErrorCode.Expected, $"{_context.Available_Functions[i].Functions_Arguments.Count} parameters but received {func_parameters.Branches.Count}", new Location(file, line, "column")));
                        }
        ...
        {
            Semantic_Errors.Add(new Error(Error.TypeError.Semantic_Error, Error.ErrorCode.Invalid,
                "import requires UI/Infrastructure layer", new Location(file, line, "column")));
        }
        else
        {
